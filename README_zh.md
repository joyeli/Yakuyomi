...
- **裝置端 pipeline** — 偵測、去字跑 NCNN 行動核心（NEON/Vulkan），OCR 跑 int8 量化模型——CPU 上快、且 GPU/NPU-ready。只有 LLM 翻譯那步離開裝置，圖片永遠不出手機。
...
- **自備模型** — 模型集（NCNN 偵測 + 去字成對檔、int8 OCR，約 96 MB）可一鍵下載（含 sha256 驗證），或自己手動放（[模型說明](https://github.com/joyeli/yakuyomi-engine/blob/main/docs/MODELS_zh.md)）。
